...
def satellite_look(lon: np.ndarray, lat: np.ndarray, elevation: np.ndarray,
                   orbit_df: pd.DataFrame,
                   clock_start: float,
                   prf: float,
                   num_valid_az: int,
                   ...
                   earth_radius: float,
                   ra: float = 6378137.0,
                   rc: float = 6356752.31424518) -> tuple:
    """
    Compute satellite look vectors in local East-North-Up coordinates.

    Replaces GMTSAR SAT_look binary.
    ...
    Returns
    -------
    look_E, look_N, look_U : tuple of arrays
        Look vector components in local East-North-Up coordinates
        (unit vectors pointing from ground to satellite)
    ...
    t1 = 86400.0 * clock_start + (nrows - num_valid_az) / (2.0 * prf)
    t2 = t1 + num_patches * num_valid_az / prf
    ts = 2.0 / prf  # sample interval

    # Pre-compute orbit positions at sampled times
    npad = 8000
    nrec = int((t2 - t1) / ts)
    times = np.arange(nrec + 2 * npad) * ts + (t1 - npad * ts)

    # Use nval=6 to match GMTSAR SAT_llt2rat.c line 105
    orb_x = _hermite_interp(orbit_time, px, vx, times, nval=6)
    orb_y = _hermite_interp(orbit_time, py, vy, times, nval=6)
    orb_z = _hermite_interp(orbit_time, pz, vz, times, nval=6)
    ...
    # GMTSAR SAT_llt2rat.c uses the input elevation directly without adjustment
    xp, yp, zp = geodetic_to_ecef(lon, lat, elevation)

    # Find closest approach for each point using golden section search
    n_points = len(lon)
    look_E = np.zeros(n_points)
    look_N = np.zeros(n_points)
    look_U = np.zeros(n_points)

    # Find satellite position at closest approach for each point
    # Use simple brute-force search on sampled orbit (fast enough for typical use)
    n_orb = len(orb_x)

    # Compute all distances (n_points x n_orb) - use chunking for memory efficiency
    chunk_size = 1000
    xmin = np.zeros(n_points, dtype=int)

    for i in range(0, n_points, chunk_size):
        end = min(i + chunk_size, n_points)
        # Distances from this chunk of points to all orbit positions
        dx = xp[i:end, np.newaxis] - orb_x[np.newaxis, :]
        dy = yp[i:end, np.newaxis] - orb_y[np.newaxis, :]
        dz = zp[i:end, np.newaxis] - orb_z[np.newaxis, :]
        distances = np.sqrt(dx**2 + dy**2 + dz**2)
        xmin[i:end] = np.argmin(distances, axis=1)
    ...
    # Unit look vector (ground to satellite) in ECEF
    dx = sat_x - xp
    dy = sat_y - yp
    dz = sat_z - zp
    ...
    # Rotation matrices: first Rz by (lon+90), then Rx by (lat-90)
    lon_rad = np.radians(np.mod(lon + 360.0, 360.0))
    lat_rad = np.radians(lat)

    # Combined rotation: A = Rx @ Rz
    b = lat_rad - np.pi/2  # (lat - 90) in radians
    g = lon_rad + np.pi/2  # (lon + 90) in radians
    ...
    # A[0,0]*unit_x + A[0,1]*unit_y + A[0,2]*unit_z  (East component)
    # A[1,0]*unit_x + A[1,1]*unit_y + A[1,2]*unit_z  (North component)
    # A[2,0]*unit_x + A[2,1]*unit_y + A[2,2]*unit_z  (Up component)

    # Rx = [[1, 0, 0], [0, cos_b, -sin_b], [0, sin_b, cos_b]]
    # Rz = [[cos_g, sin_g, 0], [-sin_g, cos_g, 0], [0, 0, 1]]
    # A = Rx @ Rz
    # A[0,:] = [cos_g, sin_g, 0]
    # A[1,:] = [cos_b*(-sin_g), cos_b*cos_g, -sin_b]
    # A[2,:] = [sin_b*(-sin_g), sin_b*cos_g, cos_b]

    look_E = cos_g * unit_x + sin_g * unit_y
    look_N = -sin_g * cos_b * unit_x + cos_g * cos_b * unit_y - sin_b * unit_z
    look_U = -sin_g * sin_b * unit_x + cos_g * sin_b * unit_y + cos_b * unit_z

    return look_E.reshape(original_shape), look_N.reshape(original_shape), look_U.reshape(original_shape)
